...
from src.langgraph_setup import (  # Assuming 'members' is also exposed
    master_agent_graph,
    members,
)
...
def convert_to_langchain_messages(history: List[Dict[str, str]]) -> List[BaseMessage]:
    ...
    for item in history:
        ...
        if role == "user":
            ...
            role == "ai" or role == "assistant"
        ):  # Accept both "ai" and "assistant" for flexibility
...
async def master_chat(request: ChatRequest):
    ...
    if request.agent:
        ...
        langchain_history[
            -1
        ].content += f" (User has specified intent for agent: {request.agent.value})"
    ...
    if (
        ...
        ai_response_content = ai_response_content.replace(
            "[upload_required]", ""
        ).strip()
    elif (
