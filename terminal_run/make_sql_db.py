...
from sqlalchemy import inspect, func, types
    ...
    with engine.connect() as connection:
        ...
        for column in table.columns:
            ...
            column_profile['null_percentage'] = (
                (column_profile['null_count'] / total_rows * 100) if total_rows > 0 else 0
            )
 
            # Distinct count
            distinct_count_query = select(func.count(func.distinct(column))).select_from(table)
            ...
            if isinstance(column.type, (types.Integer, types.Numeric, types.Float)):
                ...
                ).select_from(table).where(column.isnot(None)) # Exclude NULLs from calculations
            ...
            # Date Ranges (Date, DateTime, Timestamp)
                ...
                ).select_from(table).where(column.isnot(None)) # Exclude NULLs
            ...
            # Text/String Metrics & Enumerations
                ...
                ).select_from(table).where(column.isnot(None)) # Exclude NULLs from length calc
                ...
                if column_profile['distinct_percentage'] < 1.0 and column_profile['distinct_count'] > 0:
                    enumeration_values_query = select(
                        func.distinct(column)
                    ).select_from(table).where(column.isnot(None)).order_by(column) # Order for consistency
                    enumeration_values = connection.execute(enumeration_values_query).scalars().all()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Please check your DB_USER, DB_PASSWORD, DB_HOST, DB_PORT, and DB_NAME configurations.")
        exit(1)
    ...
    if not all_table_names:
        print(f"No tables found in database '{DB_NAME}'. Please ensure tables are created and visible to the user.")
        exit(0)
    ...
    for table_name in all_table_names:
        ...
        for col_name, col_profile in profile_data['columns'].items():
            ...
            if 'numeric_stats' in col_profile and col_profile['numeric_stats']['min_value'] is not None:
                ...
                print(f"      StdDev: {col_profile['numeric_stats']['stddev_value']:.2f}")
            elif 'date_range' in col_profile and col_profile['date_range']['min_date'] is not None:
                print(f"    Date Range:")
                print(f"      Min Date: {col_profile['date_range']['min_date']}")
                print(f"      Max Date: {col_profile['date_range']['max_date']}")
            elif 'string_lengths' in col_profile and col_profile['string_lengths']['min_length'] is not None:
                print(f"    String Lengths:")
                print(f"      Min Length: {col_profile['string_lengths']['min_length']}")
                print(f"      Max Length: {col_profile['string_lengths']['max_length']}")
                print(f"      Avg Length: {col_profile['string_lengths']['avg_length']:.2f}")
 
            if col_profile.get('is_enumeration'):
                print(f"    Enumeration Candidate (Unique values < 1%):")
                print(f"      Values: {', '.join(map(str, col_profile['enumeration_values']))}")
