...
def get_glossary(plsql_script, file):
    ...
    for table in tables.split(", "):
        table_parts = re.split(r'[_\.]', table.upper())
        # Create a boolean mask for rows where any part of the table name matches
        mask = glossary_df['Table Name'].str.upper().apply(
            lambda x: any(part in str(x) for part in table_parts) if pd.notna(x) else False
        )
        plsql_table_glossary = glossary_df[mask][['Table Name', 'Table Description', 'Column Name', 'Column Description']]
